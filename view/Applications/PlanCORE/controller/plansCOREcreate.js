sap.ui.define([
	...
	return ManagedObject.extend("pwc.portal.eval.ClubEvaluations.view.Applications.PlanCORE.controller.plansCOREcreate", {
		club_dms_id: '',
		constructor: function (oView) {
			this._oView = oView;
			this._oControl = sap.ui.xmlfragment(oView.getId(),
				"pwc.portal.eval.ClubEvaluations.view.Applications.PlanCORE.view.plansCOREcreate", this);
			this._bInit = false;
		},
		...
		_onCancel: function () {
			var self = this;
			var sPath;
			sPath = this._oControl.getBindingContext("plansCORE").getPath();
			sPath = sPath.replace('/', '');
			if (this._oControl.getModel("plansCORE").getPendingChanges()[sPath]) {
				this._oControl.getModel("plansCORE").resetChanges([this._oControl.getBindingContext("plansCORE").getPath()], undefined, /*bDeleteCreatedEntities*/
					true);
			}
		...
		handleUploadPress: function (oEvent) {
			...
			var oModel = this._oControl.getModel("plansCORE");

			var file = oEvent.getParameter("files")[0];
			var sBindingPath = oEvent.getSource().getBindingContext("plansCORE").getPath(); // oEvent.get
