sap.ui.define([
	...
	return ManagedObject.extend("pwc.portal.eval.ClubEvaluations.view.Applications.MediaPlan55a.controller.Create55a", {
		club_dms_id: '',

		constructor: function (oView) {
			this._oView = oView;
			this._oControl = sap.ui.xmlfragment(oView.getId(), "pwc.portal.eval.ClubEvaluations.view.Applications.MediaPlan55a.view.Create55a",
				this);
			this._bInit = false;
		},
		...
		_onSubmit: function (oEvent) {
			...
			if (status) {
				...
				var oModel = this.getView().getModel("mediaPlan");

				var sBindingPath = oEvent.getSource().getBindingContext("mediaPlan").sPath;
		...
		_onCancel: function () {
			var self = this;
			var sPath;
			sPath = this._oControl.getBindingContext("mediaPlan").getPath();
			sPath = sPath.replace('/', '');
			if (this._oControl.getModel("mediaPlan").getPendingChanges()[sPath]) {
				this._oControl.getModel("mediaPlan").resetChanges([this._oControl.getBindingContext("mediaPlan").getPath()], undefined, /*bDeleteCreatedEntities*/
					true);
			}
		...
		handleUploadPress: function (oEvent) {
			...
			var oModel = this._oControl.getModel("mediaPlan");

			var file = oEvent.getParameter("files")[0];
			var sBindingPath = oEvent.getSource().getBindingContext("mediaPlan").getPath(); // oEvent.get
		...
		onInit: function () {
			...
			this.oModel = this.getOwnerComponent().getModel("mediaPlan");
			var that = this
		...
		_getRequiredFields: function () {
			...
			return [{
				"id": "kpi_proof_cmis_id"
			}, {
				"id": "execution_plan_proof_cmis_id"
			}, {
				"id": "related_strategicgoal_proof_cmis_id"
			}, {
				"id": "digitl_club_accounts_proof_cmis_id"
			}];
		},
