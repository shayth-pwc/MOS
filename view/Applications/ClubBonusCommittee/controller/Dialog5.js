sap.ui.define([
	...
	return ManagedObject.extend("pwc.portal.eval.ClubEvaluations.view.Applications.ClubBonusCommittee.view.Dialog5", {
		constructor: function(oView) {
			this._oView = oView;
			this._oControl = sap.ui.xmlfragment(oView.getId(), "pwc.portal.eval.ClubEvaluations.view.Applications.ClubBonusCommittee.view.Dialog5", this);
			this._bInit = false;
		},
								...
								sap.m.MessageBox.confirm(oResourceBundle.getText("ConfirmEditSubmit"), {
									...
									onClose: function (sActionClicked) {
										if (sActionClicked === oResourceBundle.getText("Confirm")) {
											...
											oModel.resetChanges(); // oBusy.close();
										}
									}
								});
							}).catch(function (err) {
		...
		var oResourceBundle = this.getView().getModel("i18n").getResourceBundle();
			...
			if (oModel.hasPendingChanges()) {
				MessageBox.confirm(oResourceBundle.getText('ConfirmCancel'), {
					onClose: function (oAction) {
						if (oAction === sap.m.MessageBox.Action.OK) {
							that.getView().getModel("ClubBonusCommittee").resetChanges();
							that.getView().getModel("ClubBonusCommittee").refresh();
				...
				this.getView().getModel("ClubBonusCommittee").resetChanges();
		...
		onInit: function() {
			...
			this.oModel = this.getOwnerComponent().getModel("ClubBonusCommittee");
			var that= this; 
			...
			this.getOwnerComponent().getModel("ClubBonusCommittee").read("/MAINITEMS", {
                success: function (oData) {
					...
					that.getView().byId("dialog1").bindElement("ClubBonusCommittee>/MAINITEMS(" + oData.results.slice(-1).pop().id + "l)");
                     that.getView().byId("item_description").bindElement("ClubBonusCommittee>/MAINITEMS(" + oData.results.slice(-1).pop().id + "l)");
                },
