sap.ui.define([
	...
	"./CopyOfStakeholderActivities",

	"./utilities",
	"sap/m/MessageBox"
], function (BaseController, JSONModel, formatter, Filter, FilterOperator, StakeholderIdentification, StakeholderActivities,
	CopyOfStakeholderActivities, Utilities,
	MessageBox) {
	"use strict";
	return BaseController.extend("pwc.portal.eval.ClubEvaluations.view.Applications.StakeholderActivities.controller.Worklist", {
		formatter: formatter,
		/* =========================================================== */
		/* lifecycle methods                                           */
		/* =========================================================== */
		...
		/**
		 *@memberOf pwc.portal.eval.ClubEvaluations.view.Applications.StakeholderActivities.controller.Worklist
		 */
		...
		addnewactivity: function (oEvent) {
			...
			var oContext = this.getView().getModel("StakeholderActivities").createEntry("/ACTIVITIES");
			oDialog._oControl.setBindingContext(oContext, "StakeholderActivities");
			oDialog.open();
		},
		editactivity: function (oEvent) {
			...
			var context = oEvent.getSource().getBindingContext("StakeholderActivities");
			oDialog._oControl.setBindingContext(context, "StakeholderActivities");
			oDialog.open();
		},
		deleteactivity: function (oEvent) {
			var oResourceBundle = this.getView().getModel("i18n").getResourceBundle();
			var that = this,
				sPath = oEvent.getSource().getParent().getBindingContext("StakeholderActivities").getPath(),
		 ...
		 *@memberOf pwc.portal.eval.ClubEvaluations.view.Applications.StakeholderActivities.controller.Worklist
		 */
		viewrow: function (oEvent) {
			var sDialogName = "CopyOfStakeholderActivities";
			this.mDialogs = this.mDialogs || {};
			var oDialog = this.mDialogs[sDialogName];
			if (!oDialog) {
				oDialog = new CopyOfStakeholderActivities(this.getView());
				this.mDialogs[sDialogName] = oDialog;
				// For navigation.
				oDialog.setRouter(this.oRouter);
			}
