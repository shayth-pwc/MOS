sap.ui.define([
	...
	return ManagedObject.extend("pwc.portal.eval.ClubEvaluations.view.Applications.CommercialBusinessPlan.controller.Dialog1", {
		constructor: function(oView) {
			this._oView = oView;
			this._oControl = sap.ui.xmlfragment(oView.getId(), "pwc.portal.eval.ClubEvaluations.view.Applications.CommercialBusinessPlan.view.Dialog1", this);
			this._bInit = false;
		},
			...
			this.getOwnerComponent().getModel("CommercialPlan").read("/COMMERCIALSITUATION", {
                success: function (oData) {
                    ...
                    // console.log(oData.results.slice(-1).pop().id);
                     that.getView().byId("dialogname").bindElement("CommercialPlan>/COMMERCIALSITUATION(" + oData.results.slice(-1).pop().id + "l)");
                     that.getView().byId("current_situation").bindElement("CommercialPlan>/COMMERCIALSITUATION(" + oData.results.slice(-1).pop().id + "l)");
                      that.getView().byId("distinguishing_features").bindElement("CommercialPlan>/COMMERCIALSITUATION(" + oData.results.slice(-1).pop().id + "l)");
