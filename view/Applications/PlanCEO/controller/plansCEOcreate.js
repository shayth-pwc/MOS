sap.ui.define([
	...
	return ManagedObject.extend("pwc.portal.eval.ClubEvaluations.view.Applications.PlanCEO.controller.plansCEOcreate", {
		club_dms_id: '',
		constructor: function (oView) {
			this._oView = oView;
			this._oControl = sap.ui.xmlfragment(oView.getId(), "pwc.portal.eval.ClubEvaluations.view.Applications.PlanCEO.view.plansCEOcreate",
				this);
			this._bInit = false;
		},
		...
		_onCancel: function () {
			var self = this;
			var sPath;
			sPath = this._oControl.getBindingContext("plansCEO").getPath();
			sPath = sPath.replace('/', '');
			if (this._oControl.getModel("plansCEO").getPendingChanges()[sPath]) {
				this._oControl.getModel("plansCEO").resetChanges([this._oControl.getBindingContext("plansCEO").getPath()], undefined, /*bDeleteCreatedEntities*/
					true);
			}
		...
		handleUploadPress: function (oEvent) {
			...
			var oModel = this._oControl.getModel("plansCEO");

			var file = oEvent.getParameter("files")[0];
			var sBindingPath = oEvent.getSource().getBindingContext("plansCEO").getPath(); // oEvent.get
		...
		onInit: function () {
			...
			this.oModel = this.getOwnerComponent().getModel("plansCEO");
