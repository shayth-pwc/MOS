sap.ui.define([
	...
	return ManagedObject.extend("pwc.portal.eval.ClubEvaluations.view.Applications.socialevidence52G.controller.CreateSocialEvidence", {
		club_dms_id: '',
		constructor: function (oView) {
			this._oView = oView;
			this._oControl = sap.ui.xmlfragment(oView.getId(),
				"pwc.portal.eval.ClubEvaluations.view.Applications.socialevidence52G.view.CreateSocialEvidence", this);
			this._bInit = false;
		},
		...
		_onSubmit: function (oEvent) {
			...
			if (status) {
				...
				var oModel = oView.getModel("socialevidence");

				var sBindingPath = oEvent.getSource().getBindingContext("socialevidence").sPath;
				oModel.setProperty(sBindingPath + "/target_date", new Date());
				oModel.setProperty(sBindingPath + "/human_resources_number", '0');
				oModel.setProperty(sBindingPath + "/expense_allocation", '0');
				oModel.setProperty(sBindingPath + "/finance_resource", '0');
				oModel.setProperty(sBindingPath + "/related_department", '0');

				oModel.setProperty(sBindingPath + "/activity_description", new Date());
				// oModel.setProperty(sBindingPath + "/activity_type", '0');
				oModel.setProperty(sBindingPath + "/strategic_goal", '0');
				oModel.setProperty(sBindingPath + "/kpi_id", '0');
				oModel.setProperty(sBindingPath + "/target_percentage", '0');
				oModel.setProperty(sBindingPath + "/end_date_time", new Date());
				oModel.setProperty(sBindingPath + "/start_date_time", new Date());
		...
		handleUploadPress: function (oEvent) {
			...
			var oModel = this._oControl.getModel("socialevidence");

			var file = oEvent.getParameter("files")[0];
			var sBindingPath = oEvent.getSource().getBindingContext("socialevidence").getPath(); // oEvent.get
		...
		_onCancel: function () {
			var self = this;
			var sPath;
			sPath = this._oControl.getBindingContext("socialevidence").getPath();
			sPath = sPath.replace('/', '');
			if (self.oModel.getPendingChanges()[sPath]) {
				self.oModel.resetChanges([this._oControl.getBindingContext("socialevidence").getPath()], undefined, /*bDeleteCreatedEntities*/
					true);
			}
		...
		onInit: function () {
			...
			this.oModel = this.getOwnerComponent().getModel("socialevidence");
			var that = this;
