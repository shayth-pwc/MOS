sap.ui.define([
	...
	return ManagedObject.extend("pwc.portal.eval.ClubEvaluations.view.Applications.ExecutiveMeeting.controller.Create16c", {
	club_dms_id:'',
		constructor: function (oView) {
			this._oView = oView;
			this._oControl = sap.ui.xmlfragment(oView.getId(), "pwc.portal.eval.ClubEvaluations.view.Applications.ExecutiveMeeting.view.Create16c", this);
			this._bInit = false;
		},
...
/*_validateSaveEnablement: function () {
		...
		_onDialogAfterClose16c: function () {

			this.close();

		},
		_onSubmit16c: function () {
...
/*
		_onCancel16c: function () {
		console.log(this._oControl);
			var self = this;
			var sPath;
			sPath = this._oControl.getBindingContext("Executivemeeting").getPath();
			sPath = sPath.replace('/', '');
			if (this._oControl.getModel("Executivemeeting").getPendingChanges()[sPath]) {
				this._oControl.getModel("Executivemeeting").resetChanges([this._oControl.getBindingContext("Executivemeeting").getPath()], undefined,
					true);
			}
		...
		*/
		...
		_onCancel16c: function (oEvent) {
	var oResourceBundle = this.getView().getModel("i18n").getResourceBundle();

			var that = this,
				oModel = that.getView().getModel("Executivemeeting"),
			...
			if (oModel.hasPendingChanges()) {
				MessageBox.confirm(oResourceBundle.getText('ConfirmCancel'), {
					onClose: function (oAction) {
						if (oAction === sap.m.MessageBox.Action.OK) {
							that.getView().getModel("Executivemeeting").resetChanges();
							that.getView().getModel("Executivemeeting").refresh();
				...
				this.getView().getModel("Executivemeeting").resetChanges();
			...
			var oModel = this._oControl.getModel("Executivemeeting");

			var file = oEvent.getParameter("files")[0];
			var sBindingPath = oEvent.getSource().getBindingContext("Executivemeeting").getPath(); // oEvent.get
...
var that=this;
		...
		handleChangeValuestate: function (requiredFieldInfo, oView) {
			...
			var oDateTimePickerStart = this.getView().byId("meeting_start_date"),
				oDateTimePickerEnd = this.getView().byId("meeting_end_date");
