sap.ui.define([
	...
	return ManagedObject.extend("pwc.portal.eval.ClubEvaluations.view.Applications.BankPolicy42B.controller.CreateNewBankPolicy", {
		club_dms_id: '',
		constructor: function (oView) {
			this._oView = oView;
			this._oControl = sap.ui.xmlfragment(oView.getId(),
				"pwc.portal.eval.ClubEvaluations.view.Applications.BankPolicy42B.view.CreateNewBankPolicy", this);
			this._bInit = false;
		},
		...
		_onCancel: function () {
			var self = this;
			var sPath;
			sPath = this._oControl.getBindingContext("ClubFinancial").getPath();
			sPath = sPath.replace('/', '');
			if (this.oModel.getPendingChanges()[sPath]) {
				this.oModel.resetChanges([this._oControl.getBindingContext("ClubFinancial").getPath()], undefined, /*bDeleteCreatedEntities*/
					true);
			}
		...
		handleUploadPress: function (oEvent) {
			...
			var oModel = this._oControl.getModel("ClubFinancial");

			var file = oEvent.getParameter("files")[0];
			var sBindingPath = oEvent.getSource().getBindingContext("ClubFinancial").getPath(); // oEvent.get
		...
		onInit: function () {
			...
			this.oModel = this.getOwnerComponent().getModel("ClubFinancial");
			var that = this;
