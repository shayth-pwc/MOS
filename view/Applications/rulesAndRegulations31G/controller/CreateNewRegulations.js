sap.ui.define([
	...
	return ManagedObject.extend("pwc.portal.eval.ClubEvaluations.view.Applications.rulesAndRegulations31G.controller.CreateNewRegulations", {
		club_dms_id: '',
		constructor: function (oView) {
			this._oView = oView;
			this._oControl = sap.ui.xmlfragment(oView.getId(),
				"pwc.portal.eval.ClubEvaluations.view.Applications.rulesAndRegulations31G.view.CreateNewRegulations", this);
			this._bInit = false;
		},
		...
		_onSubmit: function (oEvent) {
			var oView = this.getView(),
				...
				status = true,
				requiredFieldInfo = [{
					"id": "internal_systems_development_proof_cmis_id",
					"type": "text",
					"required": true
				}, {
					"id": "sports_regulations_awareness_proof_cmis_id",
					"type": "text",
					"required": true
				}, {
					"id": "non_compliance_review_proof_cmis_id",
					"type": "text",
					"required": true
				}, {
					"id": "code_of_conduct_proof_cmis_id",
					"type": "text",
					"required": true
				}, {
					"id": "informing_stakeholders_proof_cmis_id",
					"type": "text",
					"required": true
				}, {
					"id": "internal_audit_proof_cmis_id",
					"type": "text",
					"required": true
				}];
		...
		_onCancel: function (oEvent) {
			...
			if (oModel.hasPendingChanges()) {
				MessageBox.confirm(oResourceBundle.getText('ConfirmCancel'), {
					onClose: function (oAction) {
						if (oAction === sap.m.MessageBox.Action.OK) {
							that.getView().getModel("regulations").resetChanges();
							that.getView().getModel("regulations").refresh();
				...
				this.getView().getModel("regulations").resetChanges();
		...
		handleUploadPress: function (oEvent) {
			...
			var oModel = this._oControl.getModel("regulations");

			var file = oEvent.getParameter("files")[0];
			var sBindingPath = oEvent.getSource().getBindingContext("regulations").getPath(); // oEvent.get
		...
		onInit: function () {
			...
			this.oModel = this.getOwnerComponent().getModel("regulations");
			var that = this;
