sap.ui.define([
	...
	return ManagedObject.extend("pwc.portal.eval.ClubEvaluations.view.Applications.DecisionStudy.controller.CopyOfAddDecision", {
		constructor: function (oView) {
			this._oView = oView;
			this._oControl = sap.ui.xmlfragment(oView.getId(),
				"pwc.portal.eval.ClubEvaluations.view.Applications.DecisionStudy.view.CopyOfAddDecision", this);
			this._bInit = false;
		},
		...
		_onButtonPress1: function (oEvent) {
			...
			if (oModel.hasPendingChanges()) {
				MessageBox.confirm(oResourceBundle.getText('ConfirmCancel'), {
					onClose: function (oAction) {
						if (oAction === sap.m.MessageBox.Action.OK) {
							that.getView().getModel("DecisionStudy").resetChanges();
							that.getView().getModel("DecisionStudy").refresh();
				...
				this.getView().getModel("DecisionStudy").resetChanges();
		...
		onInit: function () {
			...
			this.oModel = this.getOwnerComponent().getModel("DecisionStudy");
		...
		handleUploadPress1: function (oEvent) {
			...
			var oModel = this._oControl.getModel("DecisionStudy");

			var file = oEvent.getParameter("files")[0];
			var sBindingPath = oEvent.getSource().getBindingContext("DecisionStudy").getPath(); // oEvent.get
		...
		onSelectDecisionType: function (oEvent) {
			var oResourceBundle = this.getView().getModel("i18n").getResourceBundle();
			var DecisionType = this.getView().byId("typedecision").getText();

			// var SelectButton = this.getView().byId("myresult");

			switch (DecisionType) {
			case 'قرار عالي الخطورة':

				// SelectButton.setVisible(true);
				this.getView().byId("notneededlabel").setVisible(true);
			...
			case 'قرار استثنائي':
				...
				this.getView().byId("notneededlabel").setVisible(false);
