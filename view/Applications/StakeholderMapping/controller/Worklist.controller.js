sap.ui.define([
	...
	"./StakeholderIdentificationview",

	"./utilities",
	"sap/m/MessageBox"
], function (BaseController, JSONModel, formatter, Filter, FilterOperator, StakeholderIdentification, StakeholderIdentificationview,
	...
	return BaseController.extend("pwc.portal.eval.ClubEvaluations.view.Applications.StakeholderMapping.controller.Worklist", {
		 ...
		 */
		onInit: function () {
			var D, iOriginalBusyDelay, oTable = this.byId("table");
		...
		var oContext = this.getView().getModel("StakeholderMapping").createEntry("/STAKEHOLDERS", {
		properties: {
		id: '-1',
		club_id:'2' ,
		...
		oDialog._oControl.setBindingContext(oContext,"StakeholderMapping");
		oDialog.open();
		...
		editstakeholder: function (e) {
			...
			if (!i) {
				i = new StakeholderIdentification(this.getView());
				this.mDialogs[t] = i;
				i.setRouter(this.oRouter);
			}
			var o = e.getSource().getBindingContext("StakeholderMapping");
			i._oControl.setBindingContext(o, "StakeholderMapping");
			i.open();
		},
		deletestakeholder: function (oEvent) {
			var oResourceBundle = this.getView().getModel("i18n").getResourceBundle();
			var that = this,
				sPath = oEvent.getSource().getParent().getBindingContext("StakeholderMapping").getPath(),
		 ...
		 */
		Press1: function (e) {
			var t = "StakeholderIdentificationview";
			this.mDialogs = this.mDialogs || {};
			var i = this.mDialogs[t];
			if (!i) {
				i = new StakeholderIdentificationview(this.getView());
				this.mDialogs[t] = i;
				i.setRouter(this.oRouter);
			}
			var o = e.getSource().getBindingContext("StakeholderMapping");
