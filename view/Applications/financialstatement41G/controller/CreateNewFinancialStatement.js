sap.ui.define([
	...
	return ManagedObject.extend("pwc.portal.eval.ClubEvaluations.view.Applications.financialstatement41G.controller.CreateNewFinancialStatement", {
		club_dms_id:'',
		constructor: function (oView) {
			this._oView = oView;
			this._oControl = sap.ui.xmlfragment(oView.getId(), "pwc.portal.eval.ClubEvaluations.view.Applications.financialstatement41G.view.CreateNewFinancialStatement", this);
			this._bInit = false;
		},
		...
		_onCancel: function () {
			var self = this;
			var sPath;
			sPath = this._oControl.getBindingContext("financialstatement").getPath();
			sPath = sPath.replace('/', '');
			if (this.oModel.getPendingChanges()[sPath]) {
				this.oModel.resetChanges([this._oControl.getBindingContext("financialstatement").getPath()], undefined, /*bDeleteCreatedEntities*/
					true);
			}
			...
			self.getView().byId(RequiredInfo.id).setValueState("None");
		...
		handleUploadPress: function (oEvent) {
			...
			var oModel = this._oControl.getModel("financialstatement");

			var file = oEvent.getParameter("files")[0];
			var sBindingPath = oEvent.getSource().getBindingContext("financialstatement").getPath();// oEvent.get
		...
		onInit: function () {
			...
			this.oModel = this.getOwnerComponent().getModel("financialstatement");
				var that=this;
		...
		_getRequiredFields: function () {
			...
			return [ {"id":"audituploader"},{"id":"yearlyuploader"}];
		},
