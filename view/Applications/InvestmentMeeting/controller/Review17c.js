sap.ui.define([
	...
	return ManagedObject.extend("pwc.portal.eval.ClubEvaluations.view.Applications.InvestmentMeeting.controller.Review17c", {
		constructor: function(oView) {
			this._oView = oView;
			this._oControl = sap.ui.xmlfragment(oView.getId(), "pwc.portal.eval.ClubEvaluations.view.Applications.InvestmentMeeting.view.Review17c", this);
			this._bInit = false;
		},
		...
		/*_onDeleteRow17c: function (oEvent) {

			var sPath = oEvent.getSource().getParent().getBindingContext("Investmentmeeting").getPath();
			var oResourceBundle = this.getView().getModel("i18n").getResourceBundle();
			if (sPath) {
				return new Promise(function (fnResolve, fnReject) {
					oEvent.getSource().getParent().getModel("Investmentmeeting").remove(sPath, {
					...
					oEvent.getSource().getParent().getModel("Investmentmeeting").refresh();
				}).catch(function (err) {
					if (err !== undefined) {
						MessageBox.error(err.message);
					}
		...
		_onDeleteRow17c: function (oEvent) {
			...
			var that = this;
			var sPath = oEvent.getSource().getParent().getBindingContext("Investmentmeeting").getPath();
				var oModel = that.getView().getModel("Investmentmeeting");
	...
	oDetailsPopover.close();
			...
			var context = this.getView().byId("detailsPopover").getBindingContext("Investmentmeeting");
