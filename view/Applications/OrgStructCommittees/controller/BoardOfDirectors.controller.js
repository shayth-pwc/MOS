sap.ui.define([
	...
	return BaseController.extend("pwc.portal.eval.ClubEvaluations.view.Applications.OrgStructCommittees.controller.BoardOfDirectors", {
		pathid: 0,
		club_dms_id: '',
		onInit: function () {
		...
		handleComExecswitch: function () {
			...
			this.getOwnerComponent().getModel("Evalcommittee").read("/CLUB_ORG_STRUC_ExecutiveCommittee", {
				success: function (oData) {
					...
					that.pathid = path.id;
					that.getView().byId("BlockLayout4").setVisible(true);
		...
		handleComBoardswitch: function () {
			...
			this.getOwnerComponent().getModel("Evalcommittee").read("/CLUB_ORG_STRUC_BoardofDirectorsCommittee", {
				success: function (oData) {
					...
					that.pathid = path.id;
					that.getView().byId("BlockLayout4").setVisible(true);
					// that.getView().byId("AreofSpecialityvalue1").setVisible(true);
					// that.getView().byId("AreofSpecialityinput1").setVisible(true);

					that.getView().bindElement("Evalcommittee>/CLUB_ORG_STRUC_BoardofDirectorsCommittee(" + path.id + "l)");
					// that.getView().byId("com_cmis_id").bindElement("Evalcommittee>/CLUB_ORG_STRUC_BoardofDirectorsCommittee(" + oData.results.slice(-
					// 	1).pop().id + "l)");
					// that.getView().byId("urlboard").bindElement("Evalcommittee>/CLUB_ORG_STRUC_BoardofDirectorsCommittee(" + oData.results.slice(-
					// 	1).pop().id + "l)");
					// that.getView().byId("pdfboard").bindElement("Evalcommittee>/CLUB_ORG_STRUC_BoardofDirectorsCommittee(" + oData.results.slice(-
					// 	1).pop().id + "l)");
		...
		// 	// var sBindingPath = oEvent.getSource().getBindingContext("Evalcommittee").sPath;
		...
		onUploadcomlist: function (oEvent) {
			...
			var file = oEvent.getParameter("files")[0];
			var sBindingPath = "/CLUB_ORG_STRUC_COM_Upload(" + this.pathid + "l)";
			var datapath = oEvent.getSource().oFilePath.oParent.sId.split('--')[2];
